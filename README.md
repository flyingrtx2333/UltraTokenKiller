...
- RTK：211 项全部仍待完整的固定上游变体证据；其中 `read`、`json`、`smart`、`grep`、`rg` 仅通过固定上游成功样例，仍缺失败、未知格式和适用平台证据；其余 206 项尚无逐变体成功样例。60 项完成现有契约映射，151 项未验证。契约映射和未知格式透传均不计为对标通过。命令族统计见 [RTK 命令覆盖分母](docs/rtk-command-denominator-2026-09-20.md)。
