...
能力状态的源码绑定和自动降级规则见 [能力证据绑定](docs/capability-evidence-2026-09-20.md)。当前 33/36 个核心能力及 22/22 个已审阅命令契约通过离线套件并绑定当前源码；211 个固定 RTK 命令变体已全部纳入覆盖分母；55 项已映射现有审阅契约，156 项仍待逐项验证。分母来源、命令族统计和剩余证据要求见 [RTK 命令覆盖分母](docs/rtk-command-denominator-2026-09-20.md)。剩余 3 项是 Caveman 成对质量及 Hermes CLI/gateway 真实验收，不能以透传或测试文件存在冒充通过。
