...
type FullParityState = 'not_implemented'|'contract_mapped'|'offline_passed'|'fixed_upstream_partial_passed'|'fixed_upstream_full_passed'|'real_client_passed'|'asset_not_ready'
type CapabilityReport = {summary:Record<CapabilityState,number>;total_core_capabilities:number;full_parity_denominator:number;full_parity_verification_summary:Record<FullParityState,number>;discovered_command_variants:number;command_inventory_coverage_denominator:number;command_inventory_status_counts:Record<string,number>;command_inventory_exclusions:number;reviewed_command_contracts:number;verified_command_contracts:number;parity_certified:boolean;upstream_comparisons_missing:boolean;verification:{ledger_present:boolean;suite_passed:boolean;generated_at?:string;source_bound_capabilities:number;real_client_source_bound:number;stale_capabilities:string[];full_parity_source_current:number;full_parity_evidence_current:number;full_parity_evidence_present:number;full_parity_asset_not_ready:number}}
  ...
  return <>
    ...
    <main id="main">
      ...
      <section aria-labelledby="overview-title">
        ...
        <div className="savings-note"><strong>256 项统一证据</strong><span>固定上游完整 {readiness?.full_parity_verification_summary.fixed_upstream_full_passed??0}、部分 {readiness?.full_parity_verification_summary.fixed_upstream_partial_passed??0}、离线 {readiness?.full_parity_verification_summary.offline_passed??0}、契约映射 {readiness?.full_parity_verification_summary.contract_mapped??0}、未实现 {readiness?.full_parity_verification_summary.not_implemented??0}、真实客户端 {readiness?.full_parity_verification_summary.real_client_passed??0}、资产未就绪 {readiness?.full_parity_verification_summary.asset_not_ready??0}。证据当前 {readiness?.verification.full_parity_evidence_current??0}/{readiness?.full_parity_denominator??256}。</span></div>
