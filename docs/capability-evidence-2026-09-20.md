...
- 命令契约 25 项：25 项已审阅，覆盖输出过滤器、原生文件工具、执行层和契约测试。
- 固定 RTK 的 211 个命令变体已冻结为覆盖分母；60 项映射现有审阅契约，151 项仍待逐项验证。映射不等于实现或上游对标通过。
...
- RTK 文件搜索第一批中 `read`、`json`、`smart`、`grep`、`rg` 已有固定上游成功样例，但仍缺固定上游失败、未知格式和完整平台证据，因此只标记 `fixed_upstream_success_only`，不计入 0/211 的完整通过数。
