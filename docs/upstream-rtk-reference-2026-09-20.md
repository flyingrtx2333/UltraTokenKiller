...
下表列出测试、诊断与 Git 的主要捕获输出场景；完整报告当前包含 33 项：
...
这 33 个样例只是 RTK 命令族对标的早期证据，其中包含文件列表、`rg`／`grep`、原生 `read`／`json`／`smart`，以及 Git 八个子命令的部分形态。`commit`、`pull`、`checkout`、`push`、`fetch` 已增加成功和失败捕获输出，但仍缺未知格式与其他平台固定二进制证据。它们不能代表已完成现有 211 个发现项，也不能证明 GitHub CLI、其他测试框架、云基础设施工具或平台取消语义已经完成固定上游对标。
