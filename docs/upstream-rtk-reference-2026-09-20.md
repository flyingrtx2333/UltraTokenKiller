...
下表列出测试、诊断、Git 与代码托管命令的主要捕获输出场景；完整报告当前包含 66 项：
...
| GitHub PR 列表 | 76.5% | 75.6% | 98.9% | 通过 |
| GitLab MR 列表 | 74.8% | 73.9% | 98.9% | 通过 |
| Graphite stack log | 13.0% | 13.0% | 100.0% | 通过 |
...
这 66 个样例只是 RTK 命令族对标的阶段证据。Git 已覆盖命令级成功、失败及机器格式安全边界；`gh`、`glab` 和 `gt` 已增加固定上游成功、失败及适用的显式机器格式透传。它们不能代表已完成现有 211 个发现项；其他测试框架、云基础设施工具和平台取消语义仍需逐项对标。
