...
2026-09-20 在提交 `219d043` 上完成最新 macOS arm64 隔离回归：Python 3.11.16 下 203 项测试通过；实际打包网页的 Logo、Favicon、API、SSE、桌面及窄屏布局均由 Playwright 验证。检测到 Codex 0.153.4 与 Hermes Agent 0.21.0，但未启用客户端、未修改用户配置、未调用模型。详见[阶段一报告](docs/macos-arm64-stage1-2026-09-20.md)。
...
| macOS arm64 26.6.2 / Codex 0.153.4 / Hermes 0.21.0 | 提交 `219d043` 的 203 项离线测试及实际打包网页验收通过；先前 Codex Luna 真实任务证据继续保留 | 本轮未运行模型或真实 Hermes 任务；三次对照、运行时准备、自启与升级/卸载仍待验收，无正式支持声明 |
